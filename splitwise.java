...
public class splitwise {
    ...
    public static void main(String[] args) {
        User.users.add(new User(User.UserIdGenerator, "User1", "user1@gmail.com", "123", 1000));
        User.UserIdGenerator++;
        User.users.add(new User(User.UserIdGenerator, "User2", "user2@gmail.com", "123", 1000));
        User.UserIdGenerator++;
...
class User {
    ...
    // expence = [ { typeOfExpence, gId, eID, NameOfExpence, totalExpence,
    // createdBy, Datetime, [u1,u2,u3] } ]
    // group = [ { gId: 0, gName: "foodie", Members : [0,1,2,3] } ]
    ...
    static void nonGrpExpDetails(int eid) {
        ...
        for (int i = 0; i < groups.get(expenses.get(eid).groupId).Members.size(); i++) {
            System.out.printf("\n|\n------ " + users.get(groups.get(expenses.get(eid).groupId).Members.get(i)).Name
                    + " " + dues.get(temp[i]).ExpAmount + " " + dues.get(temp[i]).Status + " "
                    + dues.get(temp[i]).DDateTime);
        }
        System.out.print("\nPress enter to continue");
        try {
            System.in.read();
